...
const createApolloLink = (session: CognitoUserSession): ApolloLink => {
  const config: UrlInfo = {
    url: process.env.REACT_APP_DIDACTIC_BARNACLE_AWS_APPSYNC_URL!,
    region: process.env.REACT_APP_DIDACTIC_BARNACLE_AWS_APPSYNC_REGION!,
    auth: {
  ...
  const httpLink = new HttpLink({
    uri: process.env.REACT_APP_DIDACTIC_BARNACLE_AWS_APPSYNC_URL!,
    fetch: customFetch,
